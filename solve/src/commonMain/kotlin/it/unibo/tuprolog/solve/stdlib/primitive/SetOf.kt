...
object SetOf : TernaryRelation.NonBacktrackable<ExecutionContext>("setof") {
    override fun Solve.Request<ExecutionContext>.computeOne(first: Term, second: Term, third: Term): Solve.Response {
        ...
        if (second.toString().contains("^")) {
            return replyWith(third mguWith mapped.toSet().sorted().toTerm())
        }
        ...
        // trasformo in un set in modo da eliminare le ripetizioni e riordinare la struttura
        return replyWith(third mguWith mapped.toSet().sorted().toTerm())
    }
}
