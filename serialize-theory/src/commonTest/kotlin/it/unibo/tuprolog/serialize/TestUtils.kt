...
fun <T : Theory> Serializer<T>.assertSerializationWorks(expected: String, actual: T) {
...
fun Serializer<Theory>.assertTermSerializationWorks(expected: String, actualGenerator: Scope.() -> Theory) {
    assertSerializationWorks(expected, Scope.empty().actualGenerator())
}

fun <T : Theory> Deserializer<T>.assertDeserializationWorks(expected: T, actual: String) {
    ...
    kotlin.test.assertTrue("""
        ...
        |   $deserialized
        |
    """.trimMargin()) { representationsAreEqual(expected, deserialized) }
}

fun Deserializer<Theory>.assertTermDeserializationWorks(actual: String, expectedGenerator: Scope.() -> Theory) {
    assertDeserializationWorks(Scope.empty().expectedGenerator(), actual)
}

private fun representationsAreEqual(t1: Theory, t2: Theory): Boolean {
